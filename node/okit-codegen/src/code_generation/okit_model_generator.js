...
class OkitModelGenerator extends OkitCodeGenerator {
    root_class = 'OciResourceModel'
    root_class_js = 'OciResourceModel.js'
...
// ${Object.keys(schema).filter((key) => !this.ignore_elements.includes(key)).map((key) => key + ' = undefined').join('\n        ')}
    ...
    generateConstructor(obj, depth=1, assign=' = ') {
        return Object.entries(obj).filter(([k, v]) => !this.common_elements.includes(k)).map(([k, v]) => `${this.indent(depth)}${depth === 1 ? 'this.' : ''}${k}${assign}${this.getInitialValue(v, depth, assign)}`)
